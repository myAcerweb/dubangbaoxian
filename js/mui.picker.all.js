...
(function($, window, document, undefined) {
    ...
    var MAX_EXCEED = 30 * DPR;
    var VISIBLE_RANGE = 90 * DPR;
    var DEFAULT_ITEM_HEIGHT = 40 * DPR;
    var BLUR_WIDTH = 10 * DPR;
    ...
    Picker.prototype.setItems = function(items,con) {
        var self = this;
        self.items = items || [];
        var buffer = [];
        var con = con || "text";
        self.items.forEach(function(item) {
            if (item !== null && item !== undefined) {
                buffer.push('<li>' + (item[con] || item) + '</li>');
            }
        });
            ...
            for (var i = 1; i <= layer; i++) {
                ...
                pickerElement.addEventListener('change', function(event) {
                    ...
                    if (nextPickerElement && nextPickerElement.picker) {
                        ...
                        nextPickerElement.picker.setItems(preItem.children,self.con);
                    }
                }, false);
            }
        },
        //填充数据
        setData: function(data,con,val) {
            var self = this;
            if(con){
                self.con = con;
            }
            data = data || [];
            self.pickers[0].setItems(data,con);
        },
        //获取选中的项（数组）
